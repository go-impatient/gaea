...
func (e CommentEdges) ArticlesOrErr() (*Article, error) {
	if e.loadedTypes[0] {
		if e.Articles == nil {
			// The edge articles was loaded in eager-loading,
			// but was not found.
...
// QueryArticles queries the "articles" edge of the Comment entity.
func (c *Comment) QueryArticles() *ArticleQuery {
	return (&CommentClient{config: c.config}).QueryArticles(c)
}
